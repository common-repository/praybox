...
//	THIS FUNCTION NO LONGER NEEDS TO BE CALLED. LEAVING IT FOR NOW.
...
function displayRequests($page,$permalink){
	...
	if($num_requests==0){
		$req_list_output.="<p class='pbx-text'>".PB_NO_REQ."</p>";
		$req_list_output.="</div>";
	}else{
		...
		foreach($active_requests as $a_req){
			$req_id=$a_req->id;
			$display_name=($a_req->anon!=1)? stripslashes($a_req->first_name)." ".stripslashes($a_req->last_name) : "<em>".PB_REQ_ANONYMOUS."</em>";
		...
		$req_list_output.="<div class='pbx-modal-bg'>"
			.implode("\n",$req_modals)
